...
[Using Sigma2](https://github.com/TheMEMOLab/Bio326-NMBU/blob/main/Doc/BestPracticesSigma2HPC.md)
...
[Review: the concept of current directry](https://github.com/TheMEMOLab/Bio326-NMBU/blob/main/Doc/BestPracticesSigma2HPC.md#temporary-working-directory-faster-and-more-efficient-jobs)
...
[Review: make a slurm script and run it by sbatch](https://Sigma2.nmbu.no/en/SLURM)
...
[Review: File transfer between Sigma2 and your computer](https://Sigma2.nmbu.no/en/Copydata)
...
[speciesX genome](https://ftp.ensembl.org/pub/release-111/fasta/bos_taurus/dna/Bos_taurus.ARS-UCD1.3.dna.toplevel.fa.gz)
...
minimap2 -t 8 -a -a /mnt/courses/BIO326/EUK/speciesX_genome/Bos_taurus.ARS-UCD1.3.dna.toplevel.fa.gz  /mnt/courses/BIO326/EUK/result_2024/cleaned_speciesX_demo_data.fastq.gz > speciesX.sam


# convert the sam file to bam format
samtools view -S -b speciesX.sam > speciesX0.bam

## sort the bam file
samtools sort speciesX0.bam -o speciesX.bam

# index the bam file
samtools index -M  speciesX.bam

# Variant Calling using Sniffles
sniffles --input  speciesX.bam --vcf speciesX.vcf
...
cp /mnt/courses/BIO326/EUK/result_2024/speciesX_Control.vcf  /speciesX_Control.vcf 
...
grep '^##' speciesX_Control.vcf | tail -n 20

# variants
grep -v '^##' speciesX_Control.vcf | more
...
NBVAR=$(grep -v '^##' speciesX_Control.vcf | wc -l)
...
grep -v '^##' speciesX_Control.vcf |  sed -n ${RANDOMVAR}p
...
Select "speciesX" as the reference species.




Upload: speciesX_Control.vcf - downloaded from Sigma2 or the section above as the file to investigate.
...
Select the speciesX reference genome - ARS-UCD1.2/bosTau9
